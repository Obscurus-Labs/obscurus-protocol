 ...
 */
export async function calculateLeanIMTProof(leaves, leafIndex, nLevels = 10) {
 ...
 */
export async function calculateLeanIMTRoot(leaves) {
    if (leaves.length === 0)
        return 0n;
    if (leaves.length === 1)
        return leaves[0];
 ...
 */
export async function verifyLeanIMTProof(leaf, pathIndices, siblings, expectedRoot) {
    const poseidon = await buildPoseidon();
